...
def branch_and_bound(za, zm, idx, plan, ms):
    ...
    # Untere Schranke
    lb_jobs = max(za[j] + sum(d for _, d in jobs[j][idx[j]:]) for j in range(n_jobs))
    lb_mach = max(
        zm[m] + sum(
    ...
    if all(idx[j] == len(jobs[j]) for j in range(n_jobs)):
        if ms < best_ub:
            best_ub = ms
            best_plan = plan[:]
            # print(f"Neue Lösung: {best_ub}")  # optional
        return

    # Kandidaten bestimmen (früheste Startzeit)
    candidates = []
    earliest = float("inf")
    ...
    # Priorität: Kandidaten nach Dauer absteigend
    candidates.sort(key=lambda x: -x[2])

    # Konflikte nach Maschine gruppieren
    by_machine = {}
    for c in candidates:
        by_machine.setdefault(c[1], []).append(c)

    # Deterministisch: falls nur ein Job auf Maschine → direkt abarbeiten
    for m, ops in by_machine.items():
        if len(ops) == 1:
            ...
            za2[j] = zm2[m] = s2 + d
            idx2[j] += 1
            branch_and_bound(za2, zm2, idx2, plan + [(j, m, s2, d)], max(ms, s2 + d))
            return

    # Konflikt → Branch auf alle Kandidaten auf dieser Maschine
