# E2 Variante C: Maschinenspezifisches M (stärkste LP-Relaxation)
...
UB = sum(p for job in jobs.values() for _, p in job)   # wie E1
M_m = {m: UB for m in machines}
...
for m in machines:
    ...
    for i in range(len(ops_m)):
        for j in range(i + 1, len(ops_m)):
            ...
            model.addCons(S[j1, k1] + p1 <= S[j2, k2] + M_m[m] * (1 - y))
            model.addCons(S[j2, k2] + p2 <= S[j1, k1] + M_m[m] * y)
...
def monitor_memory():
    ...
    while not stop_monitor:
        t = time.time() - monitor_start_time
...
if len(memory_log) >= 2 and elapsed_time > 0:
    area = 0.0
    ...
    avg_memory_mb = (area / elapsed_time) / (1024 ** 2)
else:
...
def plot_gantt(operations):
    ...
    ax.set_title("Job-Shop Gantt-Diagramm Maschinenspezifisches_M (E2)")
    ax.grid(True, axis="x", linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.savefig("Bilder/Diagramm Maschinenspezifisches_M (E2).png", dpi=300)
    print("Diagramm gespeichert als Maschinenspezifisches_M (E2).png")
...
def plot_memory_usage(memory_log):
    ...
    plt.xlabel("Zeit [s]")
    plt.ylabel("RAM [MB]")
    plt.title("Speicherverbrauch über die Zeit (Branch & Bound) Maschinenspezifisches_M (E2)")
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.savefig("Bilder/Speicher Maschinenspezifisches_M (E2).png", dpi=300)
    print("Speicherverlauf gespeichert als: Maschinenspezifisches_M (E2).png")
