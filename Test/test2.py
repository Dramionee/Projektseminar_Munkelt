...
def branch_and_bound_with_memo_and_progress():
    ...
    while q:
        ...
        if sk in visited and visited[sk] <= ms:
            continue
        visited[sk] = ms
        ...
        # 1: ALLE möglichen nächsten Operationen sammeln
        candidates = []
        ...
        for a in range(anzahl_auftraege):
            if idx[a] < len(auftraege[a]):
                ...
                s = max(za[a], zm[m])
                candidates.append((s, a, m, d))

        by_machine = defaultdict(list)
        for s, a, m, d in candidates:
            by_machine[m].append((s, a, m, d))

        conflict_candidates = []
        for m, ops in by_machine.items():
            if len(ops) > 1:
                conflict_candidates.extend(ops)

        # nach (Startzeit, Dauer) sortieren
        candidates.sort(key=lambda x: (x[0], x[3]))

        for (s, a, m, d) in reversed(candidates):
            za2, zm2, idx2 = za[:], zm[:], idx[:]
            za2[a] = zm2[m] = s + d
            idx2[a] += 1
            ms2 = max(ms, s + d)

            ra = max(za2[j] + sum(dd for _, dd in auftraege[j][idx2[j]:])
            ...
            # zusätzlicher job-machine bound
            rjm = 0
            for j in range(anzahl_auftraege):
                t = za2[j]
                for mi, dd in auftraege[j][idx2[j]:]:
                    t = max(t, zm2[mi]) + dd
                rjm = max(rjm, t)

            g2 = max(ms2, ra, rm, rjm)
