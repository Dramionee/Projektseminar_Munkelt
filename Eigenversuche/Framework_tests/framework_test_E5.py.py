# E5 Test, wie starkes Presolve/Propagation den Baum reduziert
# Minimal: sehr eingeschränkt, Solver kann nicht viel vorrechnen
...
def plot_gantt(operations):
    ...
    ax.set_title("Job-Shop Gantt-Diagramm ohne_viel_Presolve (E5)")
    ax.grid(True, axis="x", linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.savefig("Bilder/Diagramm ohne_viel_Presolve (E5).png", dpi=300)
    print("Diagramm gespeichert als ohne_viel_Presolve (E5).png")
...
def plot_memory_usage(memory_log):
    ...
    plt.xlabel("Zeit [s]")
    plt.ylabel("RAM [MB]")
    plt.title("Speicherverbrauch über die Zeit (Branch & Bound) ohne_viel_Presolve (E5)")
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.savefig("Bilder/Speicher ohne_viel_Presolve (E5).png", dpi=300)
    print("Speicherverlauf gespeichert als: ohne_viel_Presolve (E5).png")
