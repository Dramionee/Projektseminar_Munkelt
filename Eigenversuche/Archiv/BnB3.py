# Versuch Carlier und Pinson zu verbessern, ohne Startzeitsortierung
...
def carlier_pinson_safe_bound(za, zm, idx):
    lb = 0

    # --- Job-basierter CP-Bound (Head + Proc + Tail) ---
    for j in range(anzahl_auftraege):
        cur = za[j]
        ops = auftraege[j]
        for k in range(idx[j], len(ops)):
            p = ops[k][1]
            tail = sum(dd for _, dd in ops[k+1:])
            lb = max(lb, cur + p + tail)
            cur += p

    # --- Maschinenbasierter Bound ---
    for m in range(anzahl_maschinen):
        remaining = 0
        for j in range(anzahl_auftraege):
            for mi, dd in auftraege[j][idx[j]:]:
                if mi == m:
                    remaining += dd
        lb = max(lb, zm[m] + remaining)
    ...
    while q:
        ...
        for a in range(anzahl_auftraege):
            if idx[a] < len(auftraege[a]):
                ...
                lb_cp = carlier_pinson_safe_bound(za2, zm2, idx2)
